...
#: calls a single question can cost. The worker bounds resume calls per IP per hour,
#: which says nothing about one reply; this does. Enforced on the server, where the run
#: that has no budget left refuses a further browser call in writing; the widget stops
#: at the same number, so an old server cannot make it loop either. Twenty leaves room
#: for an analysis that loads, computes, plots and fixes its own errors several times.
MAX_BROWSER_RUNS_PER_REPLY = 20
