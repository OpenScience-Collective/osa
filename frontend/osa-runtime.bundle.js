...
(()=>{var z=Object.freeze({UNPARSEABLE:"unparseable",SCHEME:"scheme_not_allowed",CREDENTIALS_IN_URL:"credentials_in_url",NOT_ALLOWED:"not_in_fetch_allow",TRANSPORT:"transport_not_allowed"});function H(e,t,s={}){let r={UNPARSEABLE:"unparseable",SCHEME:"scheme_not_allowed",CREDENTIALS_IN_URL:"credentials_in_url",NOT_ALLOWED:"not_in_fetch_allow"},a;try{a=new URL(String(e),s.base)}catch{return{allowed:!1,reason:r.UNPARSEABLE}}if(a.protocol!=="https:"&&a.protocol!=="http:")return{allowed:!1,reason:r.SCHEME};if(a.username||a.password)return{allowed:!1,reason:r.CREDENTIALS_IN_URL};for(let o of t||[]){let i;if(/(^|\/)\.\.?(\/|$)/.test(String(o)))continue;try{i=new URL(String(o))}catch{continue}if(i.origin!==a.origin)continue;let d=i.pathname.endsWith("/")?i.pathname:`${i.pathname}/`;if((a.pathname.endsWith("/")?a.pathname:`${a.pathname}/`).startsWith(d))return{allowed:!0}}return{allowed:!1,reason:r.NOT_ALLOWED}}function P({bootAllow:e=[]}={}){return`
  const DENY_REASON = ${JSON.stringify(z)};
  const isUrlAllowed = ${H.toString()};
...
`)}var f=Object.freeze({MAX_STDOUT_CHARS:16384,MAX_STDERR_CHARS:8192,MAX_IMAGES:3,MAX_IMAGE_BYTES:2000000,MAX_IMAGE_EDGE_PX:8192,MAX_SUMMARY_CHARS:8192}),F=Object.freeze({memory_mb:Object.freeze({default:1536,min:64}),stdout_chars:Object.freeze({default:f.MAX_STDOUT_CHARS,min:256}),stderr_chars:Object.freeze({default:f.MAX_STDERR_CHARS,min:256}),images:Object.freeze({default:f.MAX_IMAGES,min:0}),image_px:Object.freeze({default:1024,min:16}),exec_seconds:Object.freeze({default:120,min:1})});function N(e={}){let t=(s,r=Number.POSITIVE_INFINITY)=>{let{default:a,min:o}=F[s],i=e[s],d=typeof i==="number"&&Number.isFinite(i)&&Math.floor(i)>=o;return Math.min(d?Math.floor(i):a,r)};return{stdout_chars:t("stdout_chars",f.MAX_STDOUT_CHARS),stderr_chars:t("stderr_chars",f.MAX_STDERR_CHARS),images:t("images",f.MAX_IMAGES),image_px:t("image_px",f.MAX_IMAGE_EDGE_PX),image_bytes:f.MAX_IMAGE_BYTES,summary_chars:f.MAX_SUMMARY_CHARS,exec_seconds:t("exec_seconds"),memory_mb:t("memory_mb")}}function U(){return String.raw`
...
`}function k(e,t){let s=/out of memory|Cannot enlarge memory|memory access out of bounds|Aborted\(OOM\)|RangeError: Array buffer allocation failed/i;if(!t||typeof t.send!=="function")throw TypeError("createWorkerRuntime needs env.send to report anything");function r(){let n=(l)=>Array.isArray(l)&&l.every((c)=>typeof c==="string");if(typeof t.load!=="function")return"env.load is not a function";if(typeof t.seal!=="function")return"env.seal is not a function";if(!e||typeof e!=="object")return"config is missing";if(typeof e.indexURL!=="string"||e.indexURL==="")return"config.indexURL is not a non-empty string";for(let l of["preload","allowInstall","indexUrls","fetchAllow"])if(!n(e[l]))return`config.${l} is not a list of strings`;for(let l of["helpers","outputCapture","dataClient","namespaceSeal"])if(!e.python||typeof e.python[l]!=="string"||e.python[l]==="")return`config.python.${l} is not a non-empty string`;return null}let a=null,o=null,i=null,d=!1,m=(n)=>String(n&&n.message||n);function S(n,...l){let c=i.get(n);try{return c(...l)}finally{c.destroy()}}async function E(n,l){for(let c=0;c<n.length;c++){t.send({type:"progress",phase:l,package:n[c],index:c,total:n.length});let u=[];if(await a.loadPackage(n[c],{messageCallback:()=>{},errorCallback:(b)=>u.push(String(b))}),u.length>0)throw Error(`package ${n[c]} did not load: ${u.join("; ")}`)}}async function _(){let n=r();if(n!==null){t.send({type:"error",kind:"config",message:`the runtime was built with an invalid configuration: ${n}`});return}try{if(t.send({type:"progress",phase:"loading_runtime"}),a=await t.load(e.indexURL),t.send({type:"progress",phase:"runtime_loaded"}),await E(e.preload,"loading_package"),e.allowInstall.length>0){await E(["micropip"],"loading_package");let c=a.pyimport("micropip");try{for(let u=0;u<e.allowInstall.length;u++){t.send({type:"progress",phase:"installing",package:e.allowInstall[u],index:u,total:e.allowInstall.length});let b={deps:!1};if(e.indexUrls.length>0)b.index_urls=e.indexUrls;await c.install.callKwargs(e.allowInstall[u],b)}}finally{c.destroy()}}i=a.globals.get("dict")(),a.runPython(e.python.helpers,{globals:i}),a.runPython(e.python.outputCapture,{globals:i}),o=a.globals.get("dict")(),o.set("__name__","__main__"),a.runPython(e.python.dataClient,{globals:o});let l=i.get("display");o.set("display",l),l.destroy(),a.runPython(e.python.namespaceSeal),t.seal(e.fetchAllow),t.send({type:"ready",version:a.version})}catch(l){t.send({type:"error",kind:"runtime",message:m(l)})}}async function y(n){let l=n.call_id,c=Date.now(),u=(p,w)=>{d=!1,t.send(Object.assign({type:"result",call_id:l,status:p,stdout:"",stderr:"",summary:"",images:[],artifacts:[]},w,{elapsed_ms:Date.now()-c}))};if(a===null||i===null){u("error",{stderr:"[runtime] the runtime is not booted"});return}if(d){u("error",{stderr:"[runtime] another execution is already running"});return}d=!0;let b=typeof n.code==="string"?n.code:"",x;try{let p=S("_unavailable_imports",b);x=p.toJs(),p.destroy()}catch(p){u("error",{stderr:`[runtime] the import check failed: ${m(p)}`});return}if(x.length>0){let w=x.slice(0,20).map((v)=>v.length>100?`${v.slice(0,100)}...`:v).join(", ")+(x.length>20?`, and ${x.length-20} more`:"");u("denied",{stderr:`denied_import: ${w}`,summary:`Not run. This runtime has no ${w}. Only packages the community installed at startup are available, and nothing is installed on demand.`});return}let g;try{let p=i.get("_execute");try{g=JSON.parse(await p(l,b,o))}finally{p.destroy()}}catch(p){let w=m(p),v=s.test(w)?"oom":"error";u(v,{stderr:`[runtime] the execution failed outside Python: ${w}`.slice(-4000)});return}u(g.status,{stdout:g.stdout,stderr:g.stderr,summary:g.summary,images:g.images,artifacts:g.artifacts,full:g.full})}async function O(n){if(n.type==="boot"){await _();return}if(n.type==="execute"){await y(n);return}t.send({type:"error",kind:"protocol",message:`unexpected message: ${n.type}`})}return{boot:_,execute:y,handle:O}}var h=Object.freeze({IDLE:"idle",BOOTING:"booting",READY:"ready",FAILED:"failed",TERMINATED:"terminated"}),X=120000,R=Object.freeze({TIMEOUT:"timeout",WORKER_ERROR:"worker_error",RUNTIME_ERROR:"runtime_error"});function I(e){return{indexURL:`https://cdn.jsdelivr.net/pyodide/v${e.pyodide_version}/full/`,preload:e.preload||[],allowInstall:e.allow_install||[],indexUrls:e.index_urls||[],fetchAllow:e.fetch_allow||[],python:{helpers:U(),outputCapture:B(N(e.limits)),dataClient:C(),namespaceSeal:j()}}}function M(e){let t=I(e);return`
    (function () {
  ...
  `}function q(e){let t=new Blob([e],{type:"application/javascript"}),s=URL.createObjectURL(t),r=new Worker(s);return URL.revokeObjectURL(s),r}var Y=32,G=512,K=Object.freeze(["call_id","status","stdout","stderr","summary","images","artifacts","elapsed_ms"]);function A(e,t){let s=typeof e==="string"?e:e==null?"":String(e);if(t<=0)return"";if(s.length<=t)return s;let r=64;if(t<=r)return s.slice(0,t);let a=Math.floor((t-r)*3/5),o=t-r-a,i=s.length-a-o;return`${s.slice(0,a)}
... ${i} characters omitted ...
${s.slice(s.length-o)}`}function L(e,t){let s={};for(let r of K)if(e[r]!==void 0)s[r]=e[r];if("stdout"in s)s.stdout=A(s.stdout,t.stdout_chars);if("stderr"in s)s.stderr=A(s.stderr,t.stderr_chars);if("summary"in s)s.summary=A(s.summary,t.summary_chars);if("images"in s)s.images=Array.isArray(s.images)?s.images.slice(0,t.images):[];if("artifacts"in s)s.artifacts=Array.isArray(s.artifacts)?s.artifacts.slice(0,Y).map((r)=>A(r,G)):[];if("elapsed_ms"in s){let r=Number(s.elapsed_ms);s.elapsed_ms=Number.isFinite(r)&&r>0?Math.round(r):0}return s}var J=16,V=16000000;class T{#e=new Map;#t=0;#s;#r;constructor({maxCalls:e=J,maxChars:t=V}={}){this.#s=e,this.#r=t}static#a(e){let t=e.stdout.length+e.stderr.length+e.traceback.length;for(let s of e.images)t+=s.data_base64.length;return t}remember(e,t,s){let r={stdout:String(t&&t.stdout||""),stderr:String(t&&t.stderr||""),traceback:String(t&&t.traceback||""),images:Array.isArray(s)?s:[]};this.forget(e),this.#e.set(e,r),this.#t+=T.#a(r);while(this.#e.size>1&&(this.#e.size>this.#s||this.#t>this.#r))this.forget(this.#e.keys().next().value)}get(e){let t=this.#e.get(e);if(t!==void 0)this.#e.delete(e),this.#e.set(e,t);return t}forget(e){let t=this.#e.get(e);if(t!==void 0)this.#t-=T.#a(t),this.#e.delete(e)}get size(){return this.#e.size}get chars(){return this.#t}get maxCalls(){return this.#s}}var W=Object.freeze(["stdout","stderr","traceback","figures"]);class D{constructor({runtime:e,onProgress:t=()=>{},onStateChange:s=()=>{},workerFactory:r=q,bootTimeoutMs:a=X}){if(!e||typeof e.pyodide_version!=="string")throw TypeError("PyodideRuntime requires a runtime config with pyodide_version");if(typeof a!=="number"||!Number.isFinite(a)||a<=0)throw TypeError(`bootTimeoutMs must be a positive finite number, got ${a}`);this.runtime=e,this.onProgress=t,this.onStateChange=s,this.workerFactory=r,this.bootTimeoutMs=a,this.state=h.IDLE,this.failure=null,this.version=null,this._worker=null,this._bootPromise=null,this._bootTimer=null,this._settle=null,this._pending=new Map,this._callSeq=0,this.limits=N(e.limits),this.outputs=new T}get isReady(){return this.state===h.READY}get preloadsOnOpen(){return this.runtime.preload_on==="widget_open"}_setState(e,t){this.state=e,this.onStateChange(e,t)}boot(){if(this.state===h.READY)return Promise.resolve({version:this.version});if(this._bootPromise)return this._bootPromise;if(this.state===h.TERMINATED)return Promise.reject(Error("runtime is terminated; call reboot() to start a new one"));return this.failure=null,this._setState(h.BOOTING),this._bootPromise=new Promise((e,t)=>{this._settle={resolve:e,reject:t};let s;try{s=this.workerFactory(M(this.runtime))}catch(r){this._failBoot(R.WORKER_ERROR,`worker could not be constructed: ${r&&r.message}`);return}this._worker=s,s.onmessage=(r)=>this._onWorkerMessage(r.data||{}),s.onerror=(r)=>{let a=r&&r.message||"";if(this.state===h.READY){this._failRunningExecutions(a);return}this._failBoot(R.WORKER_ERROR,r&&r.message||"the worker failed to start, which a Content-Security-Policy refusal also looks like")},this._bootTimer=setTimeout(()=>{this._failBoot(R.TIMEOUT,`the Python runtime did not finish starting within ${Math.round(this.bootTimeoutMs/1000)}s. A blocked runtime looks exactly like a slow one: if this page sets a Content-Security-Policy, check that script-src includes 'wasm-unsafe-eval' and worker-src includes blob:.`)},this.bootTimeoutMs),s.postMessage({type:"boot"})}),this._bootPromise}_onWorkerMessage(e){if(this._settle===null&&(e.type==="ready"||e.type==="error"))return;if(e.type==="result"){let t=e.full,s=L(e,this.limits);if(this._pending.has(e.call_id)&&t)this.outputs.remember(e.call_id,t,s.images);if(this._settleExecution(e.call_id,s)&&e.status==="oom")this._recycle();return}if(e.type==="progress"){this.onProgress(e);return}if(e.type==="ready"){this._clearBootTimer(),this.version=e.version||null,this._setState(h.READY,{version:this.version});let t=this._settle;if(this._settle=null,this._bootPromise=null,t)t.resolve({version:this.version});return}if(e.type==="error")this._failBoot(R.RUNTIME_ERROR,e.message||"unknown runtime error")}_clearBootTimer(){if(this._bootTimer!==null)clearTimeout(this._bootTimer),this._bootTimer=null}_failBoot(e,t){this._clearBootTimer(),this.failure={kind:e,message:t},this._disposeWorker(),this._setState(h.FAILED,this.failure);let s=this._settle;if(this._settle=null,this._bootPromise=null,s){let r=Error(t);r.kind=e,s.reject(r)}}_failPending(e){let t=Array.from(this._pending.values());this._pending.clear();for(let s of t)s.reject(Error(e))}_disposeWorker(){if(this._worker){this._worker.onmessage=null,this._worker.onerror=null;try{this._worker.terminate()}catch{}this._worker=null}this._failPending("the runtime was torn down before this execution finished")}_emptyResult(e,t,s,r){return L({call_id:e,status:t,stdout:"",stderr:s,summary:"",images:[],artifacts:[],elapsed_ms:r},this.limits)}async execute(e,t={}){if(typeof e!=="string")throw TypeError(`code must be a string, got ${typeof e}`);if(await this.boot(),this.state!==h.READY||this._worker===null)throw Error("the runtime is not running; call reboot() to start a new one");let s=t.callId||`local-${++this._callSeq}`;if(this._pending.has(s))throw Error(`an execution is already in flight for call_id ${s}`);let r=Math.max(1,this.limits.exec_seconds)*1000,a=Date.now();return new Promise((o,i)=>{let d=setTimeout(()=>{this._settleExecution(s,this._emptyResult(s,"timeout",`[runtime] the code ran longer than ${this.limits.exec_seconds}s and was stopped.`,Date.now()-a)),this._recycle()},r);this._pending.set(s,{resolve:o,reject:i,timer:d,started:a});try{this._worker.postMessage({type:"execute",call_id:s,code:e})}catch(m){this._pending.delete(s),clearTimeout(d),i(m)}})}_failRunningExecutions(e){let t=Array.from(this._pending.entries());for(let[s,r]of t)this._pending.delete(s),clearTimeout(r.timer),r.resolve(this._emptyResult(s,"oom","[runtime] the Python runtime ran out of memory and was restarted. Try working on less data at a time."+(e?" ("+e+")":""),Date.now()-r.started));this._recycle()}_settleExecution(e,t){let s=this._pending.get(e);if(!s)return!1;return this._pending.delete(e),clearTimeout(s.timer),s.resolve(t),!0}_recycle(){this._clearBootTimer(),this._disposeWorker(),this.version=null,this._settle=null,this._bootPromise=null,this._setState(h.IDLE)}getFullOutput(e,t={}){let s=e&&typeof e.call_id==="string"?e.call_id:"",r=e&&e.stream||"stdout",a=e&&e.offset,o=Number.isInteger(a)&&a>0?a:0,i=JSON.stringify(s.length>256?`${s.slice(0,256)}...`:s),d=(n)=>L({call_id:t.callId||`local-${++this._callSeq}`,status:"ok",stdout:"",stderr:"",summary:"",images:[],artifacts:[],elapsed_ms:0,...n},this.limits);if(!W.includes(r))return d({status:"error",stderr:`[runtime] unknown stream ${JSON.stringify(String(r).slice(0,64))}; use one of ${W.join(", ")}.`});let m=this.outputs.get(s);if(m===void 0)return d({status:"error",stderr:`[runtime] no output is kept for call_id ${i} in this browser. Output stays in the tab that ran the code: it is gone after a reload, and only the last ${this.outputs.maxCalls} runs are kept.`});if(r==="figures"){let n=m.images.slice(0,this.limits.images);return d({images:n,summary:n.length===0?`get_full_output: call_id ${i} returned no figures.`:`get_full_output: ${n.length} figure(s) from call_id ${i}, attached.`})}let S=r==="stdout"?"stdout":"stderr",E=S==="stdout"?this.limits.stdout_chars:this.limits.stderr_chars,_=m[r];if(o>=_.length&&_.length>0)return d({summary:`get_full_output: offset ${o} is past the end of ${r} for call_id ${i}, which has ${_.length} characters.`});let y=Math.min(_.length,o+E),O=y<_.length?` More remains: call again with offset=${y}.`:" This is the end of the stream.";return d({[S]:_.slice(o,y),summary:_.length===0?`get_full_output: ${r} of call_id ${i} is empty.`:`get_full_output: ${r} of call_id ${i}, characters ${o} to ${y} of ${_.length}.${O}`})}terminate(){this._clearBootTimer(),this._disposeWorker();let e=this._settle;if(this._settle=null,this._bootPromise=null,this._setState(h.TERMINATED),e)e.reject(Error("runtime terminated before it finished starting"))}reboot(){return this.terminate(),this.state=h.IDLE,this.failure=null,this.version=null,this.boot()}}globalThis.OSARuntime=Object.freeze({PyodideRuntime:D,RUNTIME_STATE:h,buildWorkerSource:M,buildWorkerConfig:I,createWorkerRuntime:k});})();
