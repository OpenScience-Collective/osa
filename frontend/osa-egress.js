...
export function buildDataClientSource() {
  return [
    ...
    '',
    'def _build_osa_client(_fetch, _to_js, _Object):',
    ...
    '        for the caller to judge. OSError means no response arrived, which is',
    '        also what a URL outside fetch_allow and a redirect produce. headers',
    '        may carry Range and nothing else, so a byte range can be read and a',
    '        credential-bearing header cannot even be expressed.',
    '        """',
    '        sent = {}',
    ...
    '        try:',
    '            if sent:',
    '                init = _to_js({"headers": sent}, dict_converter=_Object.fromEntries)',
    '                response = await _fetch(url, init)',
    '            else:',
    '                response = await _fetch(url)',
    '            buffer = await response.arrayBuffer()',
    '        except Exception as err:',
    '            raise OSError("no response from %s: %s" % (url, err)) from err',
    ...
    '    return module',
    '',
    '_sys.modules["osa"] = _build_osa_client(_js.fetch, _to_js, _js.Object)',
    'osa = _sys.modules["osa"]',
    '',
    'del _collections, _ilu, _sys, _types, _js, _to_js, _build_osa_client',
  ].join('\n');
}
